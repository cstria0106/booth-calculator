...
// Yields orders one-by-one as soon as they are fetched.
export async function* streamOrders(
  existingIds: Set<string>,
  opts: StreamOptions = {}
): AsyncGenerator<Order, void, unknown> {
  const { signal, perOrderDelayMs = 1000, perPageDelayMs = 3000 } = opts;
  let page = 1;
  const lastPage = await getLastPage();
  while (page <= lastPage) {
